...
import { FinalsBracketVisualService } from './finals-bracket-visual.service';
import * as d3 from './../shared/custom-d3';

@Component({
    selector: 'tom-finals-bracket-visual',
    templateUrl: './finals-bracket-visual.component.html',
    styleUrls: ['./finals-bracket-visual.component.scss'],
    providers: [FinalsBracketVisualService, D3Service]
})
export class FinalsBracketVisualComponent implements AfterViewInit, OnChanges
{
    // Tournament
    @Input()
    tournament: any;
    ...
    constructor (
        ...
        private _bracketService: FinalsBracketVisualService,
    ...
    ngOnChanges()
    {
        if (this.tournament && this.tournament.FinalBrackets)
        {
            this.matchesChanged(this.tournament.FinalBrackets);
        }
    }
    ...
    updateBracket(data)
    {
        ...
        this._bracket = this._bracketService.getFinalsBracket(data, config);
